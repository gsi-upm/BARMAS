 ...
 */
package es.upm.dit.gsi.barmas.agent.assumptions;
...
public class AdvancedWAClassificatorAgent extends SimpleShanksAgent implements
	...
	private void processPendingArguments(
			DiagnosisSimulation simulation) {
						...
						if (this.areDistributionsFarEnough(receivedBelief,
							...
							try {
								if (!this.updatedBeliefs.containsKey(p
									...
									this.getLogger().warning("Duplicated belief in the same reasoning cycle for agent: " + this.getID());
								}
							} catch (ShanksException e) {
								this.getLogger()
										.warning(
