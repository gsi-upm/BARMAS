...
public class SurfaceDemoTest {
	...
	private void screenshot() {
		...
		Shape surface = Builder.buildOrthonormal(new OrthonormalGrid(range, steps, range, steps),
				mapper);
		...
		CompileableComposite surface2 = Builder.buildOrthonormalBig(new OrthonormalGrid(range,
				steps, range, steps), mapper);
		surface2.setFaceDisplayed(true);
		surface2.setWireframeDisplayed(true);
		surface2.setColorMapper(new ColorMapper(new ColorMapRainbow(), surface.getBounds()
				.getZmin(), surface.getBounds().getZmax(), new Color(1, 1, 1, 0.75f)));
		surface2.setLegend(new AWTColorbarLegend(surface2, new AxeBoxLayout()));
		Shape surface3 = Builder.buildRing(new OrthonormalGrid(range, steps, range, steps), mapper,
				1f, 1.5f);
		surface3.setFaceDisplayed(true);
		surface3.setWireframeDisplayed(true);
		surface3.setColorMapper(new ColorMapper(new ColorMapRainbow(), surface.getBounds()
				.getZmin(), surface.getBounds().getZmax(), new Color(1, 1, 1, 0.75f)));
		...
		surface4.setColorMapper(new ColorMapper(new ColorMapRainbow(), surface.getBounds()
				.getZmin(), surface.getBounds().getZmax(), new Color(1, 1, 1, 0.75f)));
